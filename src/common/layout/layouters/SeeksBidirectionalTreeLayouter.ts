...
import type {
  ...
  RGNode,
  RGOptionsFull,
  RGTreeLayoutOptions, RelationGraphInstance
...
export class SeeksBidirectionalTreeLayouter implements IGraphLayouter {
  ...
  doBeforeLayout() {
    // relationGraphDelegate.getRelationGraphView().value?.getInstance().
    const relationGraph = relationGraphDelegate.getRelationGraphView()?.value?.getInstance();
    relationGraph?.setDefaultLineShape(4); // 将连线格式设置为折线
    relationGraph?.stopAutoLayout();
    relationGraph?.setDefaultJunctionPoint("tb"); // 将连接点设置为上下
  }
